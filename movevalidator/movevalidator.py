class MoveValidator():
	...
	def add_move(self, move):
		...
		if piece == "p":
			...
			return self.__move_biship(file, rank, capture, origin_file, origin_rank)
		elif piece == "K":
	...
	def get_color_to_move(self):
		return "b" if self.__current_move % 2 == 0 else "w"

	def __move_pawn(self, file, rank, capture, origin_file):
		...
		if (capture):
			...
			files = [self.__get_index(origin_file)]
			if color == 'w':
				ranks = [r - 1]
			else:
				ranks = [r + 1]
		...
		for f in files:
			for r in ranks:
				...
				if self.__board_position[r][f] == color + "p":
					self.__update_board(f, r, new_file, new_rank)
					return True
	...
	def __move_knight(self, file, rank, capture, origin_file, origin_rank):
		return False


	def __move_rook(self, file, rank, capture, origin_file, origin_rank):
		return False


	def __move_bishop(self, file, rank, capture, origin_file, origin_rank):
		return False
	...
	def __move_queen(self, file, rank, capture, origin_file, origin_rank):
		return False
	...
	def __open_path(self, f1, r1, f2, r2):
		if f1 == f2:
			...
			smallerf = f1 if f1 < f2 else f2
			largerf = f1 if f1 > f2 else f2
			smallerr = r1 if r1 < r2 else r2

			for f in range(smallerf, largerf):
				if self.__board_position[smallerr + (f - smallerf)][f] is not None:
					return False
